...
Same_Date_OriDes_Filtered_Range = FILTER Same_Date_OriDes_Filtered BY  ((year1 == 2007 AND month1 >= 6) OR (year1 == 2008 AND month1 <= 5)) AND ((year2 == 2007 AND month2 >= 6) OR (year2 == 2008 AND month2 <= 5));


-- Calculate Delay of all valid flights
delay = FOREACH Same_Date_OriDes_Filtered_Range GENERATE (arrivalDelayMins1 + arrivalDelayMins2) as total_delay;
